...
simpleTheadHelp ( name, status, onHeaderClick ) =
    let
        content =
            case status of
                Unsortable ->
...
applySorter isReversed sorter data =
    case sorter of
        ...
        Increasing increasingSorter ->
            increasingSorter data

        Decreasing decreasingSorter ->
            List.reverse (decreasingSorter data)

        IncOrDec incOrDecSorter ->
            if isReversed then
                List.reverse (incOrDecSorter data)

            else
                incOrDecSorter data

        DecOrInc decOrIncSorter ->
            if isReversed then
                decOrIncSorter data

            else
                List.reverse (decOrIncSorter data)
